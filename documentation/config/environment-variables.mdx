...
Only one key is required to run debates — `OPENROUTER_API_KEY` routes all four default models through OpenRouter:
...
With this setup, `claude` calls go directly to the Anthropic API; `gpt`, `gemini`, and `grok` route through OpenRouter.
